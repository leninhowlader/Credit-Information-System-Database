...
-- =============================================
Alter PROCEDURE InterestSuspense
...
BEGIN
	...
	Set @VchrType = 3;
	Set @Category = 'Interest Suspense';
	Set @Remarks = '';
	Set @RollbackQuery = '';
	Set @QueryWithCommit = '';
	...
	While @@Fetch_Status = 0
		...
		If @Product = 'Home Mortgage Loan'
			...
			Select @VchrRef, ROW_NUMBER() Over (Order by A.LoanNo), B.AccCode, @GenLedCode, 
				   0, A.SuspenseAmount, A.LoanNo, A.Name  
			...
			Set @AmountSum = (Select Sum(CrAmount) From @VoucherDetail Where VchrRef = @VchrRef);
			
			Insert Into @VoucherDetail(VchrRef, SerialNo, AccCode, GenLedCode, DrAmount, CrAmount)
			Values (@VchrRef, @SerialNo, @AccCode, @GenLedCode, @AmountSum, 0);
			End
		Else If @Product = 'Project Mortgage Loan'
			Begin
		...
		Set @RollbackQuery = 'Declare @List Table (ScheduleId Varchar(11), PostDate Datetime); ' +
							 ...
							 'From Fimisdb.dbo.tbl_OverdueHistory Where isPaid = 0 And ScheduleId in ' +
							 ...
							 'When Matched Then Update Set A.isSuspended = 1;';
	...
	Where isPaid = 0 And ScheduleId in (Select ScheduleId From @SuspenseAmountList)
		And isSuspended = 1
	Group By ScheduleId;
